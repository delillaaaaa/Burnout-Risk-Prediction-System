...
const mlClient = axios.create({ baseURL: ML_BASE });
...
const mapToMLInput = (assessment: AssessmentData, profile: ProfileData) => {
  ...
  let jobRoleEnc = 5;
  if (profile.role === "Physician / Doctor") jobRoleEnc = 1;
  else if (profile.role === "Nurse / Nurse Practitioner") jobRoleEnc = 2;
  else if (profile.role === "Surgeon") jobRoleEnc = 3;
  else if (profile.role === "Psychologist / Therapist") jobRoleEnc = 4;
  ...
  const workLifeScore = assessment.workLifeBalance * 20;
  ...
  const satisfactionInverse = 100 - satisfactionLevel;
...
// ─────────────────────────────────────────────────────────────────────────────
// SCREEN 4: General Questionnaire (3-step) with onSave
// ─────────────────────────────────────────────────────────────────────────────
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-100/80 border border-border p-8">
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5"><label className="text-sm font-semibold text-slate-700">Gender</label><select value={form.gender} onChange={(e) => setForm({ ...form, gender: e.target.value })} className={selectClass}><option value="">Select...</option><option>Male</option><option>Female</option><option>Non-binary</option><option>Prefer not to say</option></select></div>
              </div>
              <div className="space-y-1.5"><label className="text-sm font-semibold text-slate-700">Job Role</label><select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className={selectClass}><option value="">Select your role...</option><option>Physician / Doctor</option><option>Nurse / Nurse Practitioner</option><option>Surgeon</option><option>Psychologist / Therapist</option><option>Hospital Administrator</option><option>Emergency Medical Technician</option><option>Pharmacist</option><option>Medical Researcher</option><option>Other Healthcare Worker</option></select></div>
              <div className="space-y-1.5"><label className="text-sm font-semibold text-slate-700">Years of Experience</label><select value={form.experience} onChange={(e) => setForm({ ...form, experience: e.target.value })} className={selectClass}><option value="">Select...</option><option>0–2 years</option><option>3–5 years</option><option>6–10 years</option><option>11–20 years</option><option>20+ years</option></select></div>
            </div>
          )}
          {step === 2 && (
            <div className="space-y-7">
              ...
              <Field label="Work Hours Per Week" type="number" placeholder="e.g. 52" value={form.hoursPerWeek} onChange={(v) => setForm({ ...form, hoursPerWeek: v })} />
              <div className="space-y-3">
          ...
          <div className="flex gap-3 mt-8">
            ...
            {step < totalSteps ? <button onClick={() => setStep(step + 1)} className="flex-1 py-3 bg-primary text-white font-bold rounded-xl hover:bg-cyan-700 shadow-lg shadow-cyan-100 flex items-center justify-center gap-2">Continue <ChevronRight className="w-4 h-4" /></button>
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-2xl font-extrabold text-slate-800">Good morning, {firstName} 👋</h1><p className="text-slate-500 text-sm mt-1">Thursday, May 15, 2026</p></div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gradient-to-br from-teal-50 to-emerald-50 rounded-2xl border border-emerald-100 p-6 shadow-sm">
              ...
              <div className="space-y-3">{["Schedule two 10-minute mindfulness breaks between shifts this week.", "Reduce overtime hours — patterns show elevated risk at 50+ hour weeks.", "Connect with a peer support colleague before end of this week."].map((tip, i) => (<div key={i} className="flex items-start gap-3"><div className="w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"><CheckCircle2 className="w-3 h-3 text-white" /></div><p className="text-sm text-slate-600">{tip}</p></div>))}</div>
            </div>
          </div>
        </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// SCREEN 6: Burnout Assessment Form (with profile and API call)
// ─────────────────────────────────────────────────────────────────────────────
function AssessmentPage({ navigate, onSubmit, profile }: { navigate: (s: Screen) => void; onSubmit: (assessment: AssessmentData, profile: ProfileData) => void; profile: ProfileData }) {
  const [step, setStep] = useState(1);
  const [form, setForm] = useState<AssessmentData>({ stress: 5, workload: 6, workLifeBalance: 3, jobSatisfaction: 3 });
  const totalSteps = 2;
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-8 overflow-auto flex items-start justify-center">
        <div className="w-full max-w-2xl">
          <div className="mb-8"><div className="flex items-center gap-2 mb-1"><button onClick={() => navigate("dashboard")} className="text-slate-400 hover:text-slate-600"><ChevronLeft className="w-5 h-5" /></button><h1 className="text-2xl font-extrabold text-slate-800">Burnout Assessment</h1></div><p className="text-slate-500 text-sm ml-7">Step {step} of {totalSteps} — Answer honestly for the most accurate results</p></div>
          <div className="mb-8 space-y-2"><div className="flex justify-between text-xs font-semibold text-slate-400"><span>{step === 1 ? "Workload & Stress" : "Satisfaction & Balance"}</span><span className="text-primary">{Math.round(pct)}% complete</span></div><div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-primary to-teal-400 rounded-full transition-all duration-500" style={{ width: `${pct}%` }} /></div></div>
          <div className="bg-white rounded-2xl border border-border shadow-sm p-8 space-y-8">
            {step === 1 && (<><div className="space-y-1"><span className="text-xs font-extrabold text-primary uppercase tracking-widest">Section 1</span><h2 className="text-xl font-extrabold text-slate-800">How are you feeling about your workload?</h2><p className="text-sm text-slate-500">Rate your current experience from 1 (very low) to 10 (extremely high)</p></div><SliderInput label="Stress Level" value={form.stress} onChange={(v) => setForm({ ...form, stress: v })} lowLabel="Calm & relaxed" highLabel="Extremely stressed" /><div className="border-t border-border" /><SliderInput label="Workload Level" value={form.workload} onChange={(v) => setForm({ ...form, workload: v })} lowLabel="Manageable" highLabel="Overwhelming" /></>)}
            {step === 2 && (<><div className="space-y-1"><span className="text-xs font-extrabold text-primary uppercase tracking-widest">Section 2</span><h2 className="text-xl font-extrabold text-slate-800">How satisfied are you with your work experience?</h2><p className="text-sm text-slate-500">Rate from 1 star (very poor) to 5 stars (excellent)</p></div><StarRating label="Work-Life Balance" value={form.workLifeBalance} onChange={(v) => setForm({ ...form, workLifeBalance: v })} /><div className="border-t border-border" /><StarRating label="Job Satisfaction" value={form.jobSatisfaction} onChange={(v) => setForm({ ...form, jobSatisfaction: v })} /></>)}
            <div className="flex gap-3 pt-2 border-t border-border">
              {step > 1 ? <button onClick={() => setStep(step - 1)} className="flex-1 py-3 border border-border text-slate-600 font-semibold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-2"><ChevronLeft className="w-4 h-4" /> Previous</button> : <button onClick={() => navigate("dashboard")} className="flex-1 py-3 border border-border text-slate-600 font-semibold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-2"><ChevronLeft className="w-4 h-4" /> Cancel</button>}
              {step < totalSteps ? <button onClick={() => setStep(step + 1)} className="flex-1 py-3 bg-primary text-white font-bold rounded-xl hover:bg-cyan-700 shadow-lg shadow-cyan-100 flex items-center justify-center gap-2">Next <ChevronRight className="w-4 h-4" /></button> : <button onClick={() => onSubmit(form, profile)} className="flex-1 py-3 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 shadow-lg shadow-emerald-100 flex items-center justify-center gap-2"><Sparkles className="w-4 h-4" /> Submit Assessment</button>}
            </div>
          </div>
...
// ─────────────────────────────────────────────────────────────────────────────
function ResultPage({ navigate, burnoutScore }: { navigate: (s: Screen) => void; burnoutScore: number }) {
  const risk = riskInfo(burnoutScore);
  const insights = burnoutScore < 35 ? ["Your current stress levels appear within clinically healthy ranges.", "Work-life balance indicators suggest a sustainable rhythm.", "Continue your current wellness practices and re-assess monthly."] : burnoutScore < 65 ? ["Moderate stress patterns detected — early intervention is recommended.", "Work hours or workload may be contributing to cumulative fatigue.", "Social support and adequate rest are critical priorities right now."] : ["High burnout risk detected — immediate action is strongly advised.", "Your stress and workload combination is critically elevated.", "Professional mental health support is recommended without delay."];
  const breakdown = [{ label: "Stress Level", score: 72 }, { label: "Workload Intensity", score: 68 }, { label: "Work-Life Balance", score: 55 }, { label: "Job Satisfaction", score: 48 }];
  return (
    <div className="flex min-h-screen bg-background">
      <Sidebar active="dashboard" navigate={navigate} />
      <main className="flex-1 p-8 overflow-auto">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center justify-between"><div><h1 className="text-2xl font-extrabold text-slate-800">Your Assessment Results</h1><p className="text-slate-500 text-sm mt-1">Assessed on May 15, 2026</p></div><button onClick={() => navigate("assessment")} className="px-4 py-2 border border-border text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-50 flex items-center gap-2"><ClipboardList className="w-4 h-4" /> Retake</button></div>
          <div className={`${risk.bg} rounded-2xl border ${risk.border} p-8`}>
            <div className="flex flex-col md:flex-row items-center gap-8"><BurnoutGauge score={burnoutScore} size="lg" /><div className="space-y-4 text-center md:text-left"><div><p className="text-xs font-extrabold text-slate-400 uppercase tracking-widest">Risk Assessment</p><h2 className={`text-3xl font-extrabold mt-1 ${risk.text}`}>{risk.label}</h2></div><p className="text-slate-600 text-sm leading-relaxed max-w-xs">{burnoutScore >= 65 ? "Our AI model has identified critical burnout markers. Immediate action and professional support are strongly recommended." : burnoutScore >= 35 ? "Elevated burnout risk detected. Proactive steps now can prevent further escalation and protect your wellbeing." : "Your risk profile looks healthy. Continue your current habits and monitor monthly to stay on track."}</p><div className="flex flex-wrap gap-3"><button onClick={() => navigate("recommendation")} className="px-5 py-2.5 bg-primary text-white text-sm font-bold rounded-xl hover:bg-cyan-700 shadow-sm shadow-cyan-200 flex items-center gap-2"><Sparkles className="w-4 h-4" /> View Recommendations</button><button onClick={() => navigate("dashboard")} className="px-5 py-2.5 border border-border bg-white text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-50 flex items-center gap-2"><Home className="w-4 h-4" /> Dashboard</button></div></div></div>
          </div>
          ...
          <div className="flex gap-3 pb-4"><button onClick={() => navigate("assessment")} className="flex-1 py-3 border border-border bg-white text-slate-600 font-semibold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-2"><ClipboardList className="w-4 h-4" /> Retake Assessment</button><button onClick={() => navigate("recommendation")} className="flex-1 py-3 bg-primary text-white font-bold rounded-xl hover:bg-cyan-700 shadow-lg shadow-cyan-100 flex items-center justify-center gap-2"><Sparkles className="w-4 h-4" /> Full Recommendations</button></div>
        </div>
      </main>
    </div>
  );
...
function RecommendationPage({ navigate, burnoutScore }: { navigate: (s: Screen) => void; burnoutScore: number }) {
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center justify-between"><div><h1 className="text-2xl font-extrabold text-slate-800">Personalized Recommendations</h1><p className="text-slate-500 text-sm mt-1">Tailored to your burnout risk profile · May 15, 2026</p></div><span className={`px-4 py-2 rounded-full text-sm font-bold ${risk.badge}`}>Score: {burnoutScore} — {risk.label}</span></div>
          <div className="bg-gradient-to-r from-primary to-teal-500 rounded-2xl p-6 text-white"><div className="flex items-center gap-3 mb-3"><div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center"><Sparkles className="w-5 h-5 text-white" /></div><div><h2 className="font-extrabold text-lg">Your AI Wellness Plan</h2><p className="text-cyan-200 text-xs">5 personalized intervention categories</p></div></div><p className="text-cyan-100 text-sm leading-relaxed">Based on your assessment, our model identified elevated stress and boundary management as your primary risk factors. These evidence-based recommendations are ordered by expected impact on your specific burnout profile.</p></div>
          <div className="space-y-5">{categories.map(({ icon: Icon, title, color, border, tips }, catIdx) => (<div key={title} className={`bg-white rounded-2xl border ${border} shadow-sm p-6 space-y-4`}><div className="flex items-center gap-3"><div className={`w-11 h-11 ${color} rounded-xl flex items-center justify-center flex-shrink-0`}><Icon className="w-5 h-5" /></div><div><h3 className="font-bold text-slate-800">{title}</h3><p className="text-xs text-slate-400">Priority {catIdx + 1}</p></div></div><div className="space-y-3">{tips.map((tip, i) => (<div key={i} className="flex items-start gap-3 p-3.5 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors"><div className="w-6 h-6 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"><span className="text-primary text-xs font-extrabold">{i + 1}</span></div><p className="text-sm text-slate-600 leading-relaxed">{tip}</p></div>))}</div></div>))}</div>
          <div className="grid grid-cols-2 gap-4">{[{ icon: "🧘", title: "Breathwork", desc: "4-7-8 breathing technique — 3 cycles before shifts" }, { icon: "🌿", title: "Nature Time", desc: "15 min outdoors daily reduces cortisol by 21%" }, { icon: "💬", title: "Talk It Out", desc: "Peer conversations reduce emotional exhaustion" }, { icon: "📵", title: "Digital Detox", desc: "No work apps after 8pm on non-call nights" }].map(({ icon, title, desc }) => (<div key={title} className="bg-white rounded-2xl border border-border p-5 shadow-sm"><div className="text-2xl mb-2">{icon}</div><h4 className="font-bold text-slate-700 text-sm">{title}</h4><p className="text-xs text-slate-400 mt-1 leading-relaxed">{desc}</p></div>))}</div>
          <div className="bg-gradient-to-br from-teal-50 to-sky-50 border border-teal-100 rounded-2xl p-8 text-center space-y-3"><Moon className="w-9 h-9 text-teal-500 mx-auto" /><p className="text-slate-700 font-medium italic text-lg leading-relaxed">"You cannot pour from an empty cup.<br />Take care of yourself first."</p><p className="text-slate-400 text-xs font-medium">BurnoutAI Wellness Reminder</p></div>
          <div className="flex gap-3 pb-6"><button onClick={() => navigate("result")} className="flex-1 py-3 border border-border bg-white text-slate-600 font-semibold rounded-xl hover:bg-slate-50 flex items-center justify-center gap-2"><ChevronLeft className="w-4 h-4" /> Back to Results</button><button onClick={() => navigate("dashboard")} className="flex-1 py-3 bg-primary text-white font-bold rounded-xl hover:bg-cyan-700 shadow-lg shadow-cyan-100 flex items-center justify-center gap-2"><Home className="w-4 h-4" /> Back to Dashboard</button></div>
        </div>
      </main>
    </div>
  );
...
export default function App() {
  ...
  const [profile, setProfile] = useState<ProfileData>({ age: "", gender: "", role: "", experience: "", hoursPerWeek: "", remoteRatio: "40" });
  ...
  const handleProfileSave = (data: ProfileData) => { setProfile(data); navigate("dashboard"); };
  const handleAssessmentSubmit = async (assessment: AssessmentData, profileData: ProfileData) => {
    try {
      ...
      const mlResponse = await mlClient.post("/predict", mlInput);
      const { burnout_score, risk_level } = mlResponse.data;
      await apiClient.post("/assessment", { user_name: userName, score: burnout_score, risk_level: risk_level, date: new Date().toISOString() });
      ...
      alert("Failed to predict or save. Make sure backend (5000) and FastAPI (8000) are running.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {screen === "dashboard" && <DashboardPage navigate={navigate} userName={userName} burnoutScore={burnoutScore} />}
      {screen === "assessment" && <AssessmentPage navigate={navigate} onSubmit={handleAssessmentSubmit} profile={profile} />}
      {screen === "loading" && <LoadingPage />}
      {screen === "result" && <ResultPage navigate={navigate} burnoutScore={burnoutScore} />}
      {screen === "recommendation" && <RecommendationPage navigate={navigate} burnoutScore={burnoutScore} />}
    </div>
  );
}